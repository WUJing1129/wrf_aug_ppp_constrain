...
/* input stream file ---------------------------------------------------------*/
static int input_strfile(const prcopt_t *opt,strfile_t *str)
{
    int type=0;
    ...
    if (str->format==STRFMT_RTCM2) {
        ...
        if ((type=input_rnxctr(opt,&str->rnx,str->fp))>=1) {
...
/* scan input files ----------------------------------------------------------*/
static int scan_file(const prcopt_t *popt,char **files, int nf, rnxopt_t *opt, strfile_t *str,
                     int *mask)
{
    eph_t  eph0 ={0,-1,-1};
    geph_t geph0={0,-1};
    ...
    for (m=0;m<nf&&!abort;m++) {
        ...
        while ((type=input_strfile(popt,str))>=-1) {
...
static void setopt_apppos(strfile_t *str, rnxopt_t *opt)
{
    ...
    if (!pntpos(0,str->obs->data,str->obs->n,str->nav,&prcopt,&sol,NULL,NULL,
                msg,NULL,NULL)) {
...
static int convrnx_s(const prcopt_t *popt,int sess, int format, rnxopt_t *opt, const char *file,
                     char **ofile)
{
    ...
    /* scan input files */
    if (!scan_file(popt,epath,nf,opt,str,mask)) {
        for (i=0;i<MAXEXFILE;i++) free(epath[i]);
        free_strfile(str);
        return 0;
    }
    ...
    for (i=0;i<nf&&!abort;i++) {
        ...
        /* input message */
        for (j=0;(type=input_strfile(popt,str))>=-1;j++) {
...
*-----------------------------------------------------------------------------*/
extern int convrnx(const prcopt_t *popt,int format, rnxopt_t *opt, const char *file, char **ofile)
{
    gtime_t t0={0};
    rnxopt_t opt_=*opt;
    double tu,ts;
    ...
    if (opt->ts.time==0||opt->te.time==0||opt->tunit<=0.0) {
        ...
        stat=convrnx_s(popt,0,format,&opt_,file,ofile);
    }
        ...
        for (i=0;;i++) { /* for each session */
            ...
            opt_.tstart=opt_.tend=t0;
            if ((stat=convrnx_s(popt,i+1,format,&opt_,file,ofile))<0) break;
        }
    }
    else {
        showmsg("no period");
