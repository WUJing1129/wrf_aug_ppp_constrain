...
extern int read_snxtrop(pppcorr_t *corr, const char *file) {
    FILE *fp;
    gtime_t time;
    ...
    while (fgets(buff, sizeof(buff), fp)) {
        ...
        if (sec == 1) {
            ...
            if (sscanf(buff, "%s %d:%d:%lf %lf %lf %lf %lf %lf %lf", sta, &year,
                       &doy, &tod, trp, std, trp + 1, std + 1, trp + 2, std + 2) < 6)
                continue;
            ep[0] = 2000.0 + year;
...
/* read wrf corrections*/
extern int read_wrftrop(sta_t* sta_, pppcorr_t* corr, const char* file) {
    FILE* fp;
    gtime_t time;
    double ep[6] = { 0, 1, 1 }, rr[3], tod, trop, trp[3] = { 0 }, std[3] = { 0 };
    char buff[1024], sta[64];
    int i, sec = 0, year, month, day, hours, minute, second;
    char* ext;

    trace(2, "read_wrftrop: file=%s\n", file);

    if (!(fp = fopen(file, "r"))) {
        trace(2, "wrf trop file read error: %s\n", file);
        return 0;
    }
    /* Add position to pppcorr_t */
    // printf("%s\n",corr->stas[0]);
    while (fgets(buff, sizeof(buff), fp)) {
        if (buff[0] == '%') {
            if (strstr(buff, "Station Name:")) {
                if (!(ext = strrchr(buff, ':'))) continue;
                ext++;
                strcpy(sta, ext);
                // printf("%s\n", sta);
            }
            ...
            matcpy(rr, sta_->pos, 3, 1);
            if (!add_pos(corr, sta, rr)) {
                printf("read_wrftrop error: add position error...");
                return 0;
            }
            continue;
        }
        sscanf(buff, "%d %d %d %d %d %d %lf", &year, &month, &day, &hours, &minute, &second, &trop);
        ep[0] = year;
        ep[1] = month;
        ep[2] = day;
        ep[3] = hours;
...
*-----------------------------------------------------------------------------*/
extern int pppcorr_read(sta_t *sta_, pppcorr_t *corr, const char *file) {
    ...
    for (i = 0; i < n; i++) {
        ...
        else if (strstr(efiles[i], "zpd") || strstr(efiles[i], "ZPD")) {
            read_snxtrop(corr, efiles[i]);
        }
        else if (strstr(efiles[i], "WRF")) {
            read_wrftrop(sta_, corr, efiles[i]);
        }
    }
...
static int get_sta(const pppcorr_t *corr, const double *pos, double max_dist,
    ...
    for (i = 0; i < corr->nsta; i++) {
        ...
        if ((dist[n] = MAX(norm(r, 3) / 1E3, 0.1)) > max_dist) 
            continue;
...
extern int pppcorr_trop(const pppcorr_t *corr, gtime_t time, const double *pos,
    ...
    if (!(n = get_sta(corr, pos, MAX_DIST_TROP, sta, dist, pos_s))) 
        return 0;
    ...
    for (i = 0; i < n; i++) {
        ...
        /* convert ztd to zwd */
        trp_s[0] -= saastamoinen(time, pos_s[i], azel, 0.0, 1,NULL,NULL);
    ...
    /* convert zwd to ztd */
    trp[0] += saastamoinen(time, pos, azel, 0.0, 1,NULL,NULL);
    return 1;
}
