...
static int decode_lextof(const unsigned char *buff, int i, gtime_t *tof,
    ...
    i += 20;
    week = getbitu(buff, i, 13);
    i += 13;
    toes = getbitu(buff, i, 16) * 15.0;
    i += 16;
...
static int decode_lexeph(const unsigned char *buff, int i, gtime_t toe,
    ...
    prn = getbitu(buff, i, 8);
    i += 8;
    eph.ura = getbitu(buff, i, 4);
    i += 4;
    eph.pos[0] = getbits_33(buff, i) * P2_6;
    i += 33;
    eph.pos[1] = getbits_33(buff, i) * P2_6;
    i += 33;
    eph.pos[2] = getbits_33(buff, i) * P2_6;
    i += 33;
    eph.vel[0] = getbits(buff, i, 28) * P2_15;
    i += 28;
    eph.vel[1] = getbits(buff, i, 28) * P2_15;
    i += 28;
    eph.vel[2] = getbits(buff, i, 28) * P2_15;
    i += 28;
    eph.acc[0] = getbits(buff, i, 24) * P2_24;
    i += 24;
    eph.acc[1] = getbits(buff, i, 24) * P2_24;
    i += 24;
    eph.acc[2] = getbits(buff, i, 24) * P2_24;
    i += 24;
    eph.jerk[0] = getbits(buff, i, 20) * P2_32;
    i += 20;
    eph.jerk[1] = getbits(buff, i, 20) * P2_32;
    i += 20;
    eph.jerk[2] = getbits(buff, i, 20) * P2_32;
    i += 20;
    eph.af0 = getbits(buff, i, 26) * P2_35;
    i += 26;
    eph.af1 = getbits(buff, i, 20) * P2_48;
    i += 20;
    eph.tgd = getbits(buff, i, 13) * P2_35;
    i += 13;
    for (j = 0; j < 7; j++) {
        eph.isc[j] = getbits(buff, i, 13) * P2_35;
        i += 13;
    }
...
static int decode_lexion(const unsigned char *buff, int i, gtime_t tof,
    ...
    ion.tspan = getbitu(buff, i, 8) * 60.0;
    i += 8; /* time span (s) */
    ion.pos0[0] = getbits(buff, i, 19) * 1E-5;
    i += 19; /* latitude  (rad) */
    ion.pos0[1] = getbits(buff, i, 20) * 1E-5;
    i += 20; /* longitude (rad) */
    ion.coef[0][0] = getbits(buff, i, 22) * 1E-3;
    i += 22;
    ion.coef[1][0] = getbits(buff, i, 22) * 1E-2;
    i += 22;
    ion.coef[2][0] = getbits(buff, i, 22) * 1E-2;
    i += 22;
    ion.coef[0][1] = getbits(buff, i, 22) * 1E-2;
    i += 22;
    ion.coef[1][1] = getbits(buff, i, 22) * 1E-2;
    i += 22;
    ion.coef[2][1] = getbits(buff, i, 22) * 1E-1;
    i += 22;
    nav->lexion = ion;
...
static int lex2rtcm(const unsigned char *msg, int i, unsigned char *buff) {
    ...
    switch ((type = getbitu(msg, i, 12))) {
        ...
        case 1057:
            ns = getbitu(msg, i + 62, 6);
            n = 68 + ns * 135;
            break; /* gps */
        case 1058:
            ns = getbitu(msg, i + 61, 6);
            n = 67 + ns * 76;
            break;
        case 1059:
            ns = getbitu(msg, i + 61, 6);
        ...
        case 1062:
            ...
            break;
        case 1063:
            ns = getbitu(msg, i + 59, 6);
            n = 65 + ns * 134;
            break; /* glonass */
        case 1064:
            ns = getbitu(msg, i + 58, 6);
            n = 64 + ns * 75;
            break;
        case 1065:
            ns = getbitu(msg, i + 58, 6);
        ...
        case 1245:
            ...
            break;
        case 1246:
            ns = getbitu(msg, i + 62, 4);
            n = 66 + ns * 133;
            break; /* qzss */
        case 1247:
            ns = getbitu(msg, i + 61, 4);
            n = 65 + ns * 74;
            break;
        case 1248:
            ns = getbitu(msg, i + 61, 4);
...
static int decode_lextype12(const lexmsg_t *msg, nav_t *nav, gtime_t *tof) {
    ...
    tow = getbitu(msg->msg, i, 20);
    i += 20;
    week = getbitu(msg->msg, i, 13);
    i += 13;
    *tof = gpst2time(week, tow);
...
extern int lexupdatecorr(const lexmsg_t *msg, nav_t *nav, gtime_t *tof) {
    ...
    switch (msg->type) {
        case 10:
            return decode_lextype10(msg, nav, tof); /* jaxa */
        case 11:
            return decode_lextype11(msg, nav, tof); /* jaxa */
        case 12:
            return decode_lextype12(msg, nav, tof); /* jaxa */
        case 20:
            return decode_lextype20(msg, nav, tof); /* gsi */
    }
...
extern int lexconvbin(int type, int format, const char *infile,
    ...
    while (fread(buff, 1, len, ifp) == len) {
        ...
        preamb = getbitu(buff, i, 32);
        i += 32;
        msg.prn = getbitu(buff, i, 8);
        i += 8;
        msg.type = getbitu(buff, i, 8);
        i += 8;
        msg.alert = getbitu(buff, i, 1);
        i += 1;
        ...
        for (j = 0; j < 212; j++) {
            msg.msg[j] = (unsigned char) getbitu(buff, i, 8);
            i += 8;
        }
        msg.msg[211] &= 0xFE;
