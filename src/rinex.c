...
//            }
            ...
            if (strstr(buff,"WIDELANE SATELLITE FRACTIONNAL BIASES")) {
                block=1;
            }
            else if (block) {
                /* cnes/cls grg/gbm clock */
                if (strstr(buff,"COMMENT")&&!strncmp(buff,"WL",2)&&(sat=satid2no(buff+3))&&
...
static int decode_obsdata(const prcopt_t *popt,FILE *fp, char *buff, double ver, int mask,
                          sigind_t *index, obsd_t *obs)
{
    ...
    char satid[8]="";
    int i,j,n,m,stat=1,p[MAXOBSTYPE],k[16],l[16],sys,prn=0;
    ...
    if (ver>2.99) { /* ver.3 */
        ...
        satno2id(obs->sat,obs->id);
        sys=satsys(obs->sat,&prn);
        if(sys==SYS_CMP&&prn>18&&!(mask&SYS_BD3)) stat=0;           /*BD2 only*/
        if(sys==SYS_CMP&&prn<=18&&popt->bd3opt==BD3OPT_BD3) stat=0; /*BD3 only*/
        if(sys==SYS_CMP&&popt->bd3opt==BD3OPT_BD3&&prn>18){
            mask|=SYS_CMP;
        }
    }
    if (!obs->sat) {
    ...
    else if (!(satsys(obs->sat,&prn)&mask)) {
        stat=0;
    }

    if(popt->geo_opt==0&&sys==SYS_CMP&&prn<6){
        stat=0;
    }
...
static int readrnxobsb(const prcopt_t *popt,FILE *fp, const char *opt, double ver, int *tsys,
...
//    mask=set_sysmask(opt);
    ...
    while (fgets(buff,MAXRNXLEN,fp)) {
        ...
        if (i==0) {
            ...
            if (decode_obsdata(popt,fp,buff,ver,mask,index,data+n)) n++;
        }
...
static int readrnxobs(const prcopt_t *popt,FILE *fp, gtime_t ts, gtime_t te, double tint,
                      ...
                      char tobs[][MAXOBSTYPE][4], obs_t *obs, sta_t *sta)
{
    ...
    /* read RINEX observation data body */
    while ((n=readrnxobsb(popt,fp,opt,ver,tsys,tobs,&flag,data,sta))>=0&&stat>=0) {
                      ...
                      nav_t *nav)
{
    ...
    trace(5,"readrnxnav: ver=%.2f sys=%d\n",ver,sys);
...
static int readrnxclk(FILE *fp, const char *opt,double ver, int index, nav_t *nav)
{
    ...
    while (fgets(buff,sizeof(buff),fp)) {
        ...
        if(ver>3.00){
            for (i=0,j=45;i<1;i++,j+=20) data[i]=str2num(buff,j,19);
        }
        else{
...
/* read RINEX file -----------------------------------------------------------*/
static int readrnxfp(const prcopt_t *popt,FILE *fp, gtime_t ts, gtime_t te, double tint,
                     ...
                     obs_t *obs, nav_t *nav, sta_t *sta)
{
    ...
    switch (*type) {
        case 'O': return readrnxobs(popt,fp,ts,te,tint,opt,index,ver,&tsys,tobs,obs,sta);
...
static int readrnxfile(const prcopt_t *popt,const char *file, gtime_t ts, gtime_t te, double tint,
                       const char *opt, int flag, int index, char *type,
                       obs_t *obs, nav_t *nav, sta_t *sta)
{
    FILE *fp;
    ...
    /* read RINEX file */
    stat=readrnxfp(popt,fp,ts,te,tint,opt,flag,index,type,obs,nav,sta);
...
*
*-----------------------------------------------------------------------------*/
extern int readrnxt(const prcopt_t *popt,const char *file, int rcv, gtime_t ts, gtime_t te,
                    ...
                    sta_t *sta)
{
    ...
    if (!*file) {
        return readrnxfp(popt,stdin,ts,te,tint,opt,0,1,&type,obs,nav,sta);
    }
    ...
    for (i=0;i<n&&stat>=0;i++) {
        stat=readrnxfile(popt,files[i],ts,te,tint,opt,0,rcv,&type,obs,nav,sta);
    }
...
extern int readrnx(const prcopt_t *popt,const char *file, int rcv, const char *opt, obs_t *obs,
                   nav_t *nav, sta_t *sta)
{
    gtime_t t={0};
    ...
    return readrnxt(popt,file,rcv,t,t,0.0,opt,obs,nav,sta);
}
...
*-----------------------------------------------------------------------------*/
extern int readrnxc(const prcopt_t *popt,const char *file, nav_t *nav)
{
    gtime_t t={0};
    ...
    /* read rinex clock files */
    for (i=0;i<n;i++) {
        if (readrnxfile(popt,files[i],t,t,0.0,"",1,index++,&type,NULL,nav,NULL)) {
            continue;
        }
        stat=0;
        break;
...
*-----------------------------------------------------------------------------*/
extern int input_rnxctr(const prcopt_t *popt,rnxctr_t *rnx, FILE *fp)
{
    eph_t eph={0};
    geph_t geph={0};
    seph_t seph={0};
    ...
    /* read RINEX OBS data */
    if (rnx->type=='O') {
        if ((n=readrnxobsb(popt,fp,rnx->opt,rnx->ver,&rnx->tsys,rnx->tobs,&flag,
