...
extern int process(prcopt_t *popt, filopt_t *fopt, solopt_t *sopt) {
    ...
    while ((file = readdir(dir)) != nullptr) {
        ...
        if(ppp){
            ...
            //if (popt->tropopt == TROPOPT_ZTD || popt->tropopt == TROPOPT_EST || popt->tropopt == TROPOPT_ESTG) {
            //    strcpy(infile[n],"E:\\PPP_AR\\wrf_aug_ar\\GNSS_DATA\\2020\\097\\products\\wrf\\JFNG_20200406_WRF_ZWD.txt");
            //    n++;
            //}
        }
...
int main(int argc,char **argv)
{
    ...
    for(int i=0;i<nday;i++){
        ...
        if(!loadprcfiles(popt_.prcdir,&popt_,&fopt_, nullptr,&nsta)) 
            return 0;
