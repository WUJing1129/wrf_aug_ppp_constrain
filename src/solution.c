...
static int decode_solxyz(char *buff, const solopt_t *opt, sol_t *sol)
{
    ...
    if(opt->outarm){
        ...
        if(i+3>n){
            for(j=0;j<3;j++){
                sol->qarm_gps[j]=(float)SQR(val[i]);i++;
            }
        }
    }
...
static int decode_solllh(char *buff, const solopt_t *opt, sol_t *sol,int coord)
{
    ...
    if(coord==INSMECH_ECEF) pos2ecef(pos,sol->rr);
    else{
        for(k=0;k<3;k++) sol->rr[k]=pos[k];
    }
    if (i<n) sol->stat=(uint8_t)val[i++];
    if (i<n) sol->ns  =(uint8_t)val[i++];
    if (i<n) sol->dop[1]  =val[i++];
    ...
    if (i+3<=n) { /* velocity */
        ...
        vel[0]=val[i++]; /* vel-e */
        vel[2]=val[i++]; /* vel-u */
        if(coord==INSMECH_ECEF) enu2ecef(pos,vel,sol->rr+3);
        else{
            for(k=0;k<3;k++) sol->rr[k+3]=vel[k];
        }
    }
    if (i+3<=n) {
...
static int decode_solins(char *buff, const solopt_t *opt, sol_t *sol,int coord)
{
    if(opt->posf==SOLF_INS_LLH){
        return decode_solllh(buff,opt,sol,coord);
    }
    else if(opt->posf==SOLF_INS_XYZ){
        return decode_solxyz(buff,opt,sol);
    }
}
...
static int decode_solpos(char *buff, const solopt_t *opt, sol_t *sol,int coord)
{
    sol_t sol0={{0}};
    char *p=buff;
    ...
    *sol=sol0;
    ...
    switch (opt->posf) {
        ...
        case SOLF_LLH : return decode_solllh(p,opt,sol,coord);
        case SOLF_ENU : return decode_solenu(p,opt,sol);
        case SOLF_INS_LLH:
        case SOLF_INS_XYZ:
                        return decode_solins(p,opt,sol,coord);
...
/* decode solution -----------------------------------------------------------*/
static int decode_sol(char *buff, const solopt_t *opt, sol_t *sol, double *rb,int coord)
{
    char *p;
    ...
    return decode_solpos(buff,opt,sol,coord);
}
...
static void decode_solopt(char *buff, solopt_t *opt)
{
    ...
    else if ((p=strstr(buff,"hgt"))) {
        opt->posf=SOLF_LLH;
        opt->degf=0;
        strncpy(opt->sep,p+3,1);
        opt->sep[1]='\0';
    }
...
extern void readsolopt(FILE *fp, solopt_t *opt)
{
                    ...
                    int qflag, const solopt_t *opt, solbuf_t *solbuf,int coord)
{
    ...
    /* decode solution */
    sol.time=solbuf->time;
    if ((stat=decode_sol((char *)solbuf->buff,opt,&sol,solbuf->rb,coord))>0) {
...
/* read solution data --------------------------------------------------------*/
extern int readsoldata(FILE *fp, gtime_t ts, gtime_t te, double tint, int qflag,
                      const solopt_t *opt, solbuf_t *solbuf,int coord)
{
    ...
    while ((c=fgetc(fp))!=EOF) {
        ...
        /* input solution */
        inputsol((uint8_t)c,ts,te,tint,qflag,opt,solbuf,coord);
    }
    return solbuf->n>0;
}
...
extern int readsolt(char *files[], int nfile, gtime_t ts, gtime_t te,
                    double tint, int qflag, solbuf_t *solbuf,int coord,const solopt_t *sopt)
{
    FILE *fp;
    solopt_t opt;
    int i;
...
#endif
        /* read solution data */
        if (!readsoldata(fp,ts,te,tint,qflag,&opt,solbuf,coord)) {
...
extern int readsol(char *files[], int nfile, solbuf_t *sol)
{
    ...
    return readsolt(files,nfile,time,time,0.0,0,sol,INSMECH_ECEF,NULL);
}
...
static int decode_solstat(char *buff, solstat_t *stat)
{
    ...
    double tow,az,el,resp,resc,snr,amb,var_amb,nv_cp,nv_pr;
    ...
    n=sscanf(buff,"$SAT%d%lf%s%d%lf%lf%lf%lf%d%lf%d%d%d%d%d%d%lf%lf%lf%lf",
             &week,&tow,id,&frq,&az,&el,&resp,&resc,&vsat,&snr,&fix,&slip,
             &lock,&outc,&slipc,&rejc,&amb,&var_amb,&nv_cp,&nv_pr);
    ...
    *stat=stat0;
    ...
    stat->amb  =(float)amb;
    stat->var_amb=(float)var_amb;
    stat->nv_cp=(float)nv_cp;
    stat->nv_pr=(float)nv_pr;
    return 1;
}
...
static void readref_ie(const char *file,solbuf_t *solbuff)
{
    int i=2,n,week;
    double sec,pos[3]={0},vn[3]={0};
    FILE *fp;
    char buff[MAXSOLMSG]={0};
    double v[50]={0};
    sol_t sol={0};

    if((fp=fopen(file,"r"))==NULL){
        trace(2,"open ie reference solution error\n");
        return;
    }

    while(fgets(buff,MAXSOLMSG,fp)){
        if(48>=buff[1]||buff[1]>=57) continue;
        if((n=tonum(buff,",",v))<3) {return;}

        week=v[0];sec=v[1];
        sol.time=gpst2time(week,sec);
        if(i+3<n){
            pos[0]=v[i++]*D2R;
            pos[1]=v[i++]*D2R;
            pos[2]=v[i++];
        ...
        i+=9;
        if(i+3<n){
            sol.ba[1]=v[i++]*MPS22MG;
            sol.ba[0]=v[i++]*MPS22MG;
            sol.ba[2]=-v[i++]*MPS22MG;
        }
        if(i+3<n+1){
            sol.bg[1]=v[i++]*DPS2DPH;
            sol.bg[0]=v[i++]*DPS2DPH;
            sol.bg[2]=-v[i++]*DPS2DPH;
        }
...
static void readref_a15(const char *file,solbuf_t *solbuff)
{
    int i=1,n,week;
    double sec,pos[3]={0},vn[3]={0};
    FILE *fp;
...
static void readref_tex(const char *file,solbuf_t *solbuff)
{
    int i=0,n,week;
    double sec,pos[3]={0},vn[3]={0};
    FILE *fp;
    ...
    while(fgets(buff,MAXSOLMSG,fp)){
        ...
        strcat(t1," ");
        strcat(t1,t2);
        str2time_atlan(t1,0,28,&sol.time);

        if((n=tonum(buff+28,"\t",v))<3){
            return;
        }
...
extern int readref(const char *file,solbuf_t *sol,const solopt_t *sopt,int week,double sow)
{
    if(sopt->ref_type==REF_SOL_FMT_IE){
        readref_ie(file,sol);
    }
    else if(sopt->ref_type==REF_SOL_FMT_A15){
        readref_a15(file,sol);
    }
    else if(sopt->ref_type==REF_SOL_FMT_POS){
        gtime_t ts={0},te={0};
        char *infile[3];
        for (int i = 0; i < 3; i++) {
        ...
        strcpy(infile[0],file);
        readsolt(infile, 1, ts, te, 0.0, 0, sol, 0,sopt);
        int i=3;
        for (; i >= 0; i--) free(infile[i]);
    }
    else if(sopt->ref_type==REF_SOL_FMT_TEX){
        readref_tex(file,sol);
    }
    return sol->n;
}
                   ...
                   const solopt_t *opt)
{
    ...
    p+=sprintf(p,"%s%s%14.4f%s%14.4f%s%14.4f%s%3d%s%3d%s%4.1f%s%8.4f%s%8.4f%s%8.4f%s"
               "%8.4f%s%8.4f%s%8.4f%s%6.2f%s%6.1f",
               s,sep,sol->rr[0],sep,sol->rr[1],sep,sol->rr[2],sep,sol->stat,sep,
               sol->ns,sep,sol->dop[1],sep,SQRT(sol->qr[0]),sep,SQRT(sol->qr[1]),sep,
                  ...
                  const solopt_t *opt,const prcopt_t *popt)
{
    ...
    p+=sprintf(p,"%s%10.4f%s%3d%s%3d%s%4.1f%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%8.4f"
               "%s%6.2f%s%6.1f",
               sep,pos[2],sep,sol->stat,sep,sol->ns,sep,sol->dop[1],sep,SQRT(Q[4]),sep,
    ...
    if(opt->outclk){
        if(popt->navsys&SYS_GPS){ /*GPS main*/
            p+=sprintf(p,"%s%12.2f",sep,sol->dtr[0]*1E9);/* GPS receiver clock offset ns*/
            if(popt->navsys&SYS_GLO){
                p+=sprintf(p,"%s%12.2f",sep,(sol->dtr[1])*1E9);/* GLO receiver clock offset ns*/
            }
            if(popt->navsys&SYS_GAL){
                p+=sprintf(p,"%s%12.2f",sep,(sol->dtr[2])*1E9);/* GAL clock offset ns*/
            }
            if(popt->navsys&SYS_CMP){
                if(popt->bd3opt<=BD3OPT_BD23){
                    p+=sprintf(p,"%s%12.2f",sep,(sol->dtr[3])*1E9);/* BDS clock offset ns*/
                }
                else if(popt->bd3opt==BD3OPT_BD2_3){
                    p+=sprintf(p,"%s%12.2f%s%12.2f",sep,(sol->dtr[3])*1E9,sep,(sol->dtr[5])*1E9);/* BD2 and BD3 clock offset ns*/
                }
                else if(popt->bd3opt==BD3OPT_BD3){
                    p+=sprintf(p,"%s%12.2f",sep,(sol->dtr[5])*1E9);/* BD3 clock offset ns*/
                }
            }
            if(popt->navsys&SYS_QZS){
                p+=sprintf(p,"%s%12.2f",sep,(sol->dtr[4])*1E9);/* QZS clock offset ns*/
            }
        }
        else if(popt->navsys&SYS_GLO){ /*GLO main*/
            p+=sprintf(p,"%s%12.2f",sep,(sol->dtr[1])*1E9);/* GLO receiver clock offset ns*/
            if(popt->navsys&SYS_GAL){
                p+=sprintf(p,"%s%12.2f",sep,(sol->dtr[2])*1E9);/* GAL clock offset ns*/
            }
        ...
        else if(popt->navsys&SYS_GAL){ /*GAL main*/
            p+=sprintf(p,"%s%12.2f",sep,(sol->dtr[2])*1E9);/* GAL clock offset ns*/
            if(popt->navsys&SYS_CMP){
                if(popt->bd3opt<=BD3OPT_BD23){
                    p+=sprintf(p,"%s%12.2f",sep,(sol->dtr[3])*1E9);/* BDS clock offset ns*/
        ...
        else if(popt->navsys&SYS_CMP){ /*BDS main*/
            if(popt->bd3opt<=BD3OPT_BD23){
                p+=sprintf(p,"%s%12.2f",sep,sol->dtr[3]*1E9);
                if(popt->navsys&SYS_QZS){
                    p+=sprintf(p,"%s%12.2f",sep,(sol->dtr[4])*1E9);/* QZS clock offset ns*/
                }
            }
            else if(popt->bd3opt==BD3OPT_BD2_3){
                p+=sprintf(p,"%s%12.2f%s%12.2f",sep,sol->dtr[3]*1E9,sep,(sol->dtr[5])*1E9);
            }
            if(popt->navsys&SYS_QZS){
                p+=sprintf(p,"%s%12.2f",sep,(sol->dtr[4])*1E9);/* QZS clock offset ns*/
            }
    ...
    if(opt->outtrp){
        p+=sprintf(p,"%s%12.2f%s%12.2f",sep,sol->ztrp[0],sep,sol->ztrp[1]);/* IRN clock offset ns*/
    }

    if(popt->ionoopt==IONOOPT_UC_CONS){
        if(popt->navsys&SYS_GPS) p+=sprintf(p,"%s%12.2f",sep,sol->rdcb[0]*1E9);
        if(popt->navsys&SYS_GLO) p+=sprintf(p,"%s%12.2f",sep,sol->rdcb[1]*1E9);
        if(popt->navsys&SYS_GAL) p+=sprintf(p,"%s%12.2f",sep,sol->rdcb[2]*1E9);
        if(popt->navsys&SYS_CMP||popt->navsys&SYS_BD3){
            if(popt->bd3opt<=BD3OPT_BD23){
                p+=sprintf(p,"%s%12.2f",sep,sol->rdcb[3]*1E9);
            }
            else if(popt->bd3opt==BD3OPT_BD2_3){
                p+=sprintf(p,"%s%12.2f%s%12.2f",sep,sol->rdcb[3]*1E9,sep,sol->rdcb[5]*1E-9);
            }
            else{
                p+=sprintf(p,"%s%12.2f",sep,sol->rdcb[5]*1E9);
            }
        }
        if(popt->navsys&SYS_QZS) p+=sprintf(p,"%s%12.2f",sep,sol->rdcb[4]*1E9);
    }

    if(popt->nf>=3&&(popt->ionoopt==IONOOPT_UC||popt->ionoopt==IONOOPT_UC_CONS)){
        if(popt->navsys&SYS_GPS) p+=sprintf(p,"%s%12.2f",sep,sol->ifcb[0]*1E9);
        if(popt->navsys&SYS_GLO) p+=sprintf(p,"%s%12.2f",sep,sol->ifcb[1]*1E9);
        if(popt->navsys&SYS_GAL) p+=sprintf(p,"%s%12.2f",sep,sol->ifcb[2]*1E9);
        if(popt->navsys&SYS_CMP) p+=sprintf(p,"%s%12.2f",sep,sol->ifcb[3]*1E9);
        if(popt->navsys&SYS_QZS) p+=sprintf(p,"%s%12.2f",sep,sol->ifcb[4]*1E9);
    }
                  ...
                  const double *rb, const solopt_t *opt)
{
    ...
    p+=sprintf(p,"%s%s%14.4f%s%14.4f%s%14.4f%s%3d%s%3d%s%4.1f%s%8.4f%s%8.4f%s%8.4f%s"
               "%8.4f%s%8.4f%s%8.4f%s%6.2f%s%6.1f\r\n",
               s,sep,enu[0],sep,enu[1],sep,enu[2],sep,sol->stat,sep,sol->ns,sep,sol->dop[1],sep,
...
extern int outprcopts(uint8_t *buff, const prcopt_t *opt)
{
    ...
    const char *s1[]={
        "Single","TDCP","DGPS","Kinematic","Static","Static_Start","Moving-Base","Fixed",
        "PPP Kinematic","PPP Static","PPP Fixed","INS-MECH","IGLC-POS",
        "IGLC-SPP","IGLC-DGPS","IGLC-PPK","IGLC-PPP",
        "IGTC-SPP","IGTC-TDCP","IGTC-DGPS","IGTC-PPK","IGTC-PPP",
        "STC-PPK","STC-PPP"
    };
    const char *s2[]={
    ...
    const char *s7[]={
        "GPS","GLONASS","Galileo","QZSS","SBAS","BDS","NavIC","","",""
    };
    const char *s8[]={
...
//    }
    ...
    for (i=0;sys[i];i++) {
        if (opt->navsys&sys[i]) {
            if(sys[i]==SYS_CMP){
                if(opt->bd3opt<=BD3OPT_BD23){
                    p+=sprintf(p," %s",s7[i]);
                }
                else if(opt->bd3opt==BD3OPT_BD2_3){
                    p+=sprintf(p," %s","BD2 BD3");
                }
                else if(opt->bd3opt==BD3OPT_BD3){
                    p+=sprintf(p," %s","BD3");
                }
            }
            else{
    ...
    for(i=0;sys[i];i++){
        if(opt->navsys&sys[i]){
            if(sys[i]==SYS_GPS){
                if(opt->nf==1){
                    p+=sprintf(p," G(%s)",obsfrqstr[i][opt->gnss_frq_idx[0][0]-1]);
                }
                else if(opt->nf==2){
                    p+=sprintf(p," G(%s+%s)",obsfrqstr[i][opt->gnss_frq_idx[0][0]-1],obsfrqstr[i][opt->gnss_frq_idx[0][1]-1]);
                }
                else if(opt->nf==3){
                    p+=sprintf(p," G(%s+%s+%s)",obsfrqstr[i][opt->gnss_frq_idx[0][0]-1],obsfrqstr[i][opt->gnss_frq_idx[0][1]-1],obsfrqstr[i][opt->gnss_frq_idx[0][2]-1]);
                }
            }
            else if(sys[i]==SYS_GLO){
                if(opt->nf==1){
                    p+=sprintf(p," R(%s)",obsfrqstr[i][opt->gnss_frq_idx[1][0]-1]);
                }
                else if(opt->nf==2){
                    p+=sprintf(p," R(%s+%s)",obsfrqstr[i][opt->gnss_frq_idx[1][0]-1],obsfrqstr[i][opt->gnss_frq_idx[1][1]-1]);
                }
            }
            else if(sys[i]==SYS_GAL){
                if(opt->nf==1){
                    p+=sprintf(p," E(%s)",obsfrqstr[i][opt->gnss_frq_idx[2][0]-1]);
                }
                else if(opt->nf==2){
                    p+=sprintf(p," E(%s+%s)",obsfrqstr[i][opt->gnss_frq_idx[2][0]-1],obsfrqstr[i][opt->gnss_frq_idx[2][1]-1]);
                }
                else if(opt->nf==3){
                    p+=sprintf(p," E(%s+%s+%s)",obsfrqstr[i][opt->gnss_frq_idx[2][0]-1],obsfrqstr[i][opt->gnss_frq_idx[2][1]-1],obsfrqstr[i][opt->gnss_frq_idx[2][2]-1]);
                }
                else if(opt->nf==4){
                    p+=sprintf(p," E(%s+%s+%s+%s)",obsfrqstr[i][opt->gnss_frq_idx[2][0]-1],obsfrqstr[i][opt->gnss_frq_idx[2][1]-1],obsfrqstr[i][opt->gnss_frq_idx[2][2]-1],obsfrqstr[i][opt->gnss_frq_idx[2][3]-1]);
                }
                else if(opt->nf==5){
                    p+=sprintf(p," E(%s+%s+%s+%s+%s)",obsfrqstr[i][opt->gnss_frq_idx[2][0]-1],obsfrqstr[i][opt->gnss_frq_idx[2][1]-1],obsfrqstr[i][opt->gnss_frq_idx[2][2]-1],obsfrqstr[i][opt->gnss_frq_idx[2][3]-1],obsfrqstr[i][opt->gnss_frq_idx[2][4]-1]);
                }
            }
            else if(sys[i]==SYS_CMP){
                if(opt->nf==1){
                    p+=sprintf(p," B2(%s)",obsfrqstr[i][opt->gnss_frq_idx[3][0]-1]);
                    if(opt->navsys&SYS_BD3) p+=sprintf(p," B3(%s)",obsfrqstr[i][opt->gnss_frq_idx[NSYS][0]-1]);
                }
                else if(opt->nf==2){
                    p+=sprintf(p," B2(%s+%s)",obsfrqstr[i][opt->gnss_frq_idx[3][0]-1],obsfrqstr[i][opt->gnss_frq_idx[3][1]-1]);
                    if(opt->navsys&SYS_BD3) p+=sprintf(p," B3(%s+%s)",obsfrqstr[i][opt->gnss_frq_idx[NSYS][0]-1],obsfrqstr[i][opt->gnss_frq_idx[NSYS][1]-1]);
                }
                else if(opt->nf==3){
                    p+=sprintf(p," B2(%s+%s+%s)",obsfrqstr[i][opt->gnss_frq_idx[3][0]-1],obsfrqstr[i][opt->gnss_frq_idx[3][1]-1],obsfrqstr[i][opt->gnss_frq_idx[3][2]-1]);
                    if(opt->navsys&SYS_BD3) p+=sprintf(p," B2(%s+%s+%s)",obsfrqstr[i][opt->gnss_frq_idx[NSYS][0]-1],obsfrqstr[i][opt->gnss_frq_idx[NSYS][1]-1],obsfrqstr[i][opt->gnss_frq_idx[NSYS][2]-1]);
                }
            }
            else if(sys[i]==SYS_QZS){
                if(opt->nf==1){
                    p+=sprintf(p," J(%s)",obsfrqstr[i][opt->gnss_frq_idx[4][0]-1]);
                }
                else if(opt->nf==2){
                    p+=sprintf(p," J(%s+%s)",obsfrqstr[i][opt->gnss_frq_idx[4][0]-1],obsfrqstr[i][opt->gnss_frq_idx[4][1]-1]);
                }
                else if(opt->nf==3){
                    p+=sprintf(p," J(%s+%s+%s)",obsfrqstr[i][opt->gnss_frq_idx[4][0]-1],obsfrqstr[i][opt->gnss_frq_idx[4][1]-1],obsfrqstr[i][opt->gnss_frq_idx[4][2]-1]);
                }
            }
        }
    }
...
* return : number of output bytes
*-----------------------------------------------------------------------------*/
extern int outsolheads(uint8_t *buff, const solopt_t *opt,const prcopt_t *popt)
{
    const char *s1[]={"WGS84","Tokyo"},*s2[]={"ellipsoidal","geodetic"};
    const char *s3[]={" GPST ","UTC ","JST "},*sep=opt2sep(opt);
    const char *leg1="Q=1:fix,2:float,3:sbas,4:dgps,5:single,6:ppp,7:ppp-ar,8:ins-align,9:ins-mech,10:gnss-lc,11:gnss-tc,12:gnss-stc,13:nhc_aid,14:zv_aid,15:dop_aid";
    const char *leg3="QG=0:none,1:sim,2:spp,3:dgps,4:ppk,5:ppk-ar,6:ppp,7:ppp-ar";
    const char *leg2="ns=# of satellites";
    char *p=(char *)buff;
    int timeu=opt->timeu<0?0:(opt->timeu>20?20:opt->timeu),ins=0;
    
    trace(4,"outsolheads:\n");
    ins=popt->mode>=PMODE_INS_MECH;
    ...
    if (opt->posf==SOLF_LLH||opt->posf==SOLF_INS_LLH) { /* lat/lon/hgt */
        if (opt->degf) {
            p+=sprintf(p,"%16s%s%16s%s%10s%s%3s%s%3s%s%4s%s%8s%s%8s%s%8s%s%8s%s%8s%s"
                       "%8s%s%6s%s%6s",
                       "latitude(d'\")",sep,"longitude(d'\")",sep,"height(m)",
                       sep,"Q",sep,"ns",sep,"pdop",sep,"sdn(m)",sep,"sde(m)",sep,"sdu(m)",
                       sep,"sdne(m)",sep,"sdeu(m)",sep,"sdue(m)",sep,"age(s)",
                       sep,"ratio");
        }
        else {
            p+=sprintf(p,"%14s%s%14s%s%10s%s%3s%s%3s%s%4s%s%8s%s%8s%s%8s%s%8s%s%8s%s"
                       "%8s%s%6s%s%6s",
                       "latitude(deg)",sep,"longitude(deg)",sep,"height(m)",sep,
                       "Q",sep,"ns",sep,"pdop",sep,"sdn(m)",sep,"sde(m)",sep,"sdu(m)",sep,
    ...
    else if (opt->posf==SOLF_XYZ||opt->posf==SOLF_INS_XYZ) { /* x/y/z-ecef */
        p+=sprintf(p,"%14s%s%14s%s%14s%s%3s%s%3s%s%4s%s%8s%s%8s%s%8s%s%8s%s%8s%s%8s"
                   "%s%6s%s%6s",
                   "x-ecef(m)",sep,"y-ecef(m)",sep,"z-ecef(m)",sep,"Q",sep,"ns",sep,"pdop",
        ...
        p+=sprintf(p,"%14s%s%14s%s%14s%s%3s%s%3s%s%4s%s%8s%s%8s%s%8s%s%8s%s%8s%s%8s"
                   ...
                   "Q",sep,"ns",sep,"pdop",sep,"sde(m)",sep,"sdn(m)",sep,"sdu(m)",sep,
                   "sden(m)",sep,"sdnu(m)",sep,"sdue(m)",sep,"age(s)",sep,
                   "ratio");
    }
    if(opt->outclk){
        if(popt->navsys&SYS_GPS){
            p+=sprintf(p,"%s%12s",sep,"G_clk");
            if(popt->navsys&SYS_GLO){
                p+=sprintf(p,"%s%12s",sep,"R_isb");
            }
            if(popt->navsys&SYS_GAL){
                p+=sprintf(p,"%s%12s",sep,"E_isb");
            }
            if(popt->navsys&SYS_CMP||popt->navsys&SYS_BD3){
                if(popt->bd3opt<=BD3OPT_BD23){
                    p+=sprintf(p,"%s%12s",sep,"C_isb");
                }
                else if(popt->bd3opt==BD3OPT_BD2_3){
                    p+=sprintf(p,"%s%12s%s%12s",sep,"B2_isb",sep,"B3_isb");
                }
                else{
                    p+=sprintf(p,"%s%12s",sep,"B3_isb");
                }
            }
            if(popt->navsys&SYS_QZS){
                p+=sprintf(p,"%s%12s",sep,"J_isb");
            }
            if(popt->navsys&SYS_IRN){
                p+=sprintf(p,"%s%12s",sep,"I_isb");
            }
        }
        else if(popt->navsys&SYS_GLO){
            p+=sprintf(p,"%s%12s",sep,"R_isb");
            if(popt->navsys&SYS_GAL){
                p+=sprintf(p,"%s%12s",sep,"E_isb");
            }
        ...
        else if(popt->navsys&SYS_GAL){
            p+=sprintf(p,"%s%12s",sep,"E_isb");
            if(popt->navsys&SYS_CMP||popt->navsys&SYS_BD3){
                if(popt->bd3opt<=BD3OPT_BD23){
                    p+=sprintf(p,"%s%12s",sep,"B_isb");
                }
                else if(popt->bd3opt==BD3OPT_BD2_3){
                    p+=sprintf(p,"%s%12s%s%12s",sep,"B2_isb",sep,"B3_isb");
                }
        ...
        else if(popt->navsys&SYS_CMP||popt->navsys&SYS_BD3){
            if(popt->bd3opt<=BD3OPT_BD23){
                p+=sprintf(p,"%s%12s",sep,"C_isb");
            }
            else if(popt->bd3opt==BD3OPT_BD2_3){
    ...
    if(opt->outtrp){
        p+=sprintf(p,"%s%12s%s%12s",sep,"ztrph",sep,"ztrpw");
    }

    if(popt->ionoopt==IONOOPT_UC_CONS){
        if(popt->navsys&SYS_GPS) p+=sprintf(p,"%s%12s",sep,"G_rdcb");
        if(popt->navsys&SYS_GLO) p+=sprintf(p,"%s%12s",sep,"R_rdcb");
        if(popt->navsys&SYS_GAL) p+=sprintf(p,"%s%12s",sep,"E_rdcb");
        if(popt->navsys&SYS_CMP||popt->navsys&SYS_BD3){
            if(popt->bd3opt<=BD3OPT_BD23){
                p+=sprintf(p,"%s%12s",sep,"C_rdcb");
            }
            else if(popt->bd3opt==BD3OPT_BD2_3){
                p+=sprintf(p,"%s%12s%s%12s",sep,"B2_rdcb",sep,"B3_rdcb");
            }
            else{
                p+=sprintf(p,"%s%12s",sep,"B3_rdcb");
            }
        }
        if(popt->navsys&SYS_QZS) p+=sprintf(p,"%s%12s",sep,"J_rdcb");
    }

    if(popt->nf>=3&&(popt->ionoopt==IONOOPT_UC||popt->ionoopt==IONOOPT_UC_CONS)){
        if(popt->navsys&SYS_GPS) p+=sprintf(p,"%s%12s",sep,"G_ifcb");
        if(popt->navsys&SYS_GLO) p+=sprintf(p,"%s%12s",sep,"R_ifcb");
        if(popt->navsys&SYS_GAL) p+=sprintf(p,"%s%12s",sep,"E_ifcb");
        if(popt->navsys&SYS_CMP||popt->navsys&SYS_BD3){
            if(popt->bd3opt<=BD3OPT_BD23){
                p+=sprintf(p,"%s%12s",sep,"C_ifcb");
            }
            else if(popt->bd3opt==BD3OPT_BD2_3){
                p+=sprintf(p,"%s%12s%s%12s",sep,"B2_ifcb",sep,"B3_ifcb");
            }
            else{
                p+=sprintf(p,"%s%12s",sep,"B3_ifcb");
            }
        }
        if(popt->navsys&SYS_QZS) p+=sprintf(p,"%s%12s",sep,"J_ifcb");
    }

    if(ins){
        p+=sprintf(p,"%s%3s",sep,"QG");
        p+=sprintf(p,"%s%14s%s%14s%s%14s%s%14s%s%14s%s%14s",sep,"roll(deg)",sep,"pitch(deg)",sep,"yaw(deg)",
                sep,"sdr(deg)",sep,"sdp(deg)",sep,"sdy(deg)");
        if(popt->insopt.est_ba&&opt->outba){
            p+=sprintf(p,"%s%14s%s%14s%s%14s%s%14s%s%14s%s%14s",sep,"bax(mg)",sep,"bay(mg)",sep,"baz(mg)",
                       sep,"sdbax(mg)",sep,"sdbay(mg)",sep,"sdbaz(mg)");
        }
        if(popt->insopt.est_bg&&opt->outbg){
            p+=sprintf(p,"%s%14s%s%14s%s%14s%s%14s%s%14s%s%14s",sep,"bgx(deg/h)",sep,"bgy(deg/h)",sep,"bgz(deg/h)",
                       sep,"sdbgx(deg/h)",sep,"sdbgy(deg/h)",sep,"sdbgz(deg/h)");
        }
        if(popt->insopt.est_sa&&opt->outsa){
            p+=sprintf(p,"%s%14s%s%14s%s%14s%s%14s%s%14s%s%14s",sep,"Sax(ppm)",sep,"Say(ppm)",sep,"Saz(ppm)",
                       sep,"sdSax(ppm)",sep,"sdSay(ppm)",sep,"sdSaz(ppm)");
        }
        if(popt->insopt.est_sg&&opt->outsg){
            p+=sprintf(p,"%s%14s%s%14s%s%14s%s%14s%s%14s%s%14s",sep,"Sgx(ppm)",sep,"Sgy(ppm)",sep,"Sgz(ppm)",
                       sep,"sdSgx(ppm)",sep,"sdSgy(ppm)",sep,"sdSgz(ppm)");
        }
        if(popt->insopt.est_armgps&&opt->outarm){
            p+=sprintf(p,"%s%14s%s%14s%s%14s%s%14s%s%14s%s%14s",sep,"Armx(m)",sep,"Army(m)",sep,"Armz(m)",
                       sep,"sdArmx(m)",sep,"sdArmy(m)",sep,"sdArmz(m)");
        }
        if(popt->insopt.est_igdt&&opt->outigdt){
            p+=sprintf(p,"%s%14s%s%14s",sep,"Igdt(s)",sep,"sdIgdt(s)");
        }
    }
                   ...
                   const solopt_t *opt,const prcopt_t *popt,const solins_t *ins_sol)
{
    ...
    if ((n=outsols(buff,sol,rb,opt,popt,ins_sol))>0) {
        fwrite(buff,n,1,fp);
    }
}
/* output solution extended ----------------------------------------------------
...
static char* outfrqinfos(const obsd_t *obs,const ssat_t *sat_info,int type,int f,double snr,int opt)
{
    static char buff[1024]={'\0'};

    if(type==0){ /*L*/
        sprintf(buff,"%5s%d lock=%5d outc=%5d slip=%d res=%7.3f norm_v=%7.3f snr=%7.3f amb=%7.3f obs=%12.3f",
                opt?"IF_L ":"L ",f,sat_info->lock[f-1],sat_info->outc[f-1],sat_info->slip[f-1],sat_info->resc[f-1],sat_info->norm_v[0][f-1],snr,sat_info->amb[f-1],obs->L[f-1]);}
    else if(type==1){ /*P*/
        sprintf(buff,"%5s%d lock=%5d outc=%5d slip=%d res=%7.3f norm_v=%7.3f snr=%7.3f amb=%7.3f obs=%12.3f",
                opt?"IF_P ":"P ",f,sat_info->lock[f-1],sat_info->outc[f-1],sat_info->slip[f-1],sat_info->resp[f-1],sat_info->norm_v[1][f-1],snr,0.0,obs->P[f-1]);
    }
    else if(type==2){ /*D*/
        sprintf(buff,"%5s%d obs=%9.3f","D ",f,obs->D[f-1]);
    }

    return buff;
}

static int outsatinfos(uint8_t *buff,const prcopt_t *popt,const rtk_t *rtk,const obsd_t *obs,int nobs)
{
    char *p=(char *)buff;
    const ssat_t *sat_info=NULL;
    const int *frq_idxs=NULL;
    int i,f,nf,sys,prn,sys_idx=-1;

    trace(4,"outsatinfos:\n");

    nf=rtk->opt.ionoopt==IONOOPT_IFLC?1:(((rtk->opt.ionoopt==IONOOPT_IF2&&rtk->opt.nf==3)?2:rtk->opt.nf));
    p+=sprintf(p,"> %s iep=%4d ns=%2d vs=%2d nf=%d stat=%s\n",
               time_str(rtk->sol.time,1),rtk->epoch,nobs,rtk->sol.ns,nf,rtk->sol.stat==SOLQ_FIX?"FIX":"FLOAT");
    for(i=0;i<nobs;i++){
        sat_info=&rtk->ssat[obs[i].sat-1];
        sys=satsys(obs[i].sat,NULL);
        sys_idx=satsysidx(obs[i].sat);
        if(sys_idx==-1) continue;
        frq_idxs=popt->gnss_frq_idx[sys_idx];

        for(f=0;f<nf;f++){
            if(f==0){
                p+=sprintf(p,"%s vs=%s el=%4.1f az=%5.1f ",
                        obs[i].id,sat_info->vs?"USE":"EXC",sat_info->azel[1]*R2D,sat_info->azel[0]*R2D);
                p+=sprintf(p,"mw=%8.2f smw=%8.2f mw_idx=%4d gf=%8.2f dmw=%5.2f mw_thres=%6.3f dgf=%6.3f gf_thres=%6.3f tec=%8.2f",
                           sat_info->mw[0],sat_info->mw[1],(int)sat_info->mw[2],sat_info->gf[0],sat_info->delta_mw[0],sat_info->delta_mw[1],sat_info->delta_gf[0],sat_info->delta_gf[1],sat_info->tec);
                p+=sprintf(p,"\n");
                p+=sprintf(p,"    %s\n",outfrqinfos(obs+i,sat_info,0,frq_idxs[0],obs[i].SNR[frq_idxs[0]-1]*0.25,(popt->ionoopt==IONOOPT_IFLC||popt->ionoopt==IONOOPT_IF2)?1:0));
                p+=sprintf(p,"    %s\n",outfrqinfos(obs+i,sat_info,1,frq_idxs[0],obs[i].SNR[frq_idxs[0]-1]*0.25,(popt->ionoopt==IONOOPT_IFLC||popt->ionoopt==IONOOPT_IF2)?1:0));
                p+=sprintf(p,"    %s\n",outfrqinfos(obs+i,sat_info,2,frq_idxs[0],obs[i].SNR[frq_idxs[0]-1]*0.25,(popt->ionoopt==IONOOPT_IFLC||popt->ionoopt==IONOOPT_IF2)?1:0));
            }
            else{
                p+=sprintf(p,"    %s\n",outfrqinfos(obs+i,sat_info,0,frq_idxs[f],obs[i].SNR[frq_idxs[f]-1]*0.25,(popt->ionoopt==IONOOPT_IFLC||popt->ionoopt==IONOOPT_IF2)?1:0));
                p+=sprintf(p,"    %s\n",outfrqinfos(obs+i,sat_info,1,frq_idxs[f],obs[i].SNR[frq_idxs[f]-1]*0.25,(popt->ionoopt==IONOOPT_IFLC||popt->ionoopt==IONOOPT_IF2)?1:0));
                p+=sprintf(p,"    %s\n",outfrqinfos(obs+i,sat_info,2,frq_idxs[f],obs[i].SNR[frq_idxs[f]-1]*0.25,(popt->ionoopt==IONOOPT_IFLC||popt->ionoopt==IONOOPT_IF2)?1:0));
            }
        }
    }
...
extern void outsatinfo(FILE *fp, const prcopt_t *popt,const rtk_t *rtk,const obsd_t *obs,int nobs)
{
    uint8_t buff[MAXSOLMSG*4+1];
    int n;

    trace(4,"outsatinfo:\n");

    if((n=outsatinfos(buff,popt,rtk,obs,nobs))){
        fwrite(buff,n,1,fp);
    }
}