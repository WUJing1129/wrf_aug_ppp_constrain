...
#define MAXINFILE   30          /* max number of input files */
                      ...
                      const solopt_t *sopt)
{
    ...
    if (sopt->outhead) {
        if (!*sopt->prog) {
            fprintf(fp,"%s program   : iRTKLIB ver.%s %s\n",COMMENTH,VER_RTKLIB,PATCH_LEVEL);
        }
        else {
                    ...
                    rtk_t *rtk, int mode)
{
    ...
    if (mode==0 || !revs || popt->modear==ARMODE_FIXHOLD)
        rtkinit(rtk,popt,NULL);
    
    rtcm_path[0]='\0';
    if(popt->mode!=PMODE_FIXED||popt->mode!=PMODE_PPP_FIXED){
        for(i=0;i<3;i++) rtk->sol.rr[i]=stas[0].pos[i];
    }
    ...
    while ((nobs=inputobs(obs,rtk->sol.stat,popt))>=0) {
        ...
        if (mode==0) { /* forward/backward */
            if (!solstatic) {
                outsol(fp,&rtk->sol,rtk->rb,sopt,&rtk->opt,NULL);
            }
...
//                num++;
//                if (!solstatic && mode == 0) {
//                    outsol(fptm,&newsol,rb,sopt,&rtk->opt);
//                }
//            }
//            oldsol = rtk->sol;
        }
    ...
    if (mode==0&&solstatic&&time.time!=0.0) {
        sol.time=time;
        outsol(fp,&sol,rb,sopt,&rtk->opt,NULL);
    }
}
...
static void combres(FILE *fp, FILE *fptm, const prcopt_t *popt, const solopt_t *sopt)
{
    ...
    for (i=0,j=isolb-1;i<isolf&&j>=0;i++,j--) {
        ...
        if (!solstatic) {
            outsol(fp,&sols,rbs,sopt,popt,NULL);
        }
        ...
        if (sols.eventime.time != 0)
        {
            ...
            num++;
            if (!solstatic) {
                outsol(fptm,&newsol,rb,sopt,popt,NULL);
            }
        }
        oldsol = sols;
    }
    if (solstatic&&time.time!=0.0) {
        sol.time=time;
        outsol(fp,&sol,rb,sopt,popt,NULL);
    }
}
/* read prec ephemeris, sbas data, lex data, tec grid and open rtcm ----------*/
extern void readpreceph(char **infile, int n, const prcopt_t *prcopt,const filopt_t *fopt,
                        nav_t *nav, sbs_t *sbs, lex_t *lex)
{
    ...
    for (i=0;i<n;i++) {
        if (strstr(infile[i],"%r")||strstr(infile[i],"%b")) continue;
        readrnxc(prcopt,infile[i],nav);
    }

    ppp=(prcopt->mode>=PMODE_PPP_KINEMA&&prcopt->mode<=PMODE_PPP_FIXED)||
            prcopt->mode==PMODE_TC_PPP||prcopt->mode==PMODE_LC_PPP?1:0;
    /* read satellite fcb files */
    if(ppp&&prcopt->modear==ARMODE_PPPAR_ILS){
        if(prcopt->arprod==AR_PROD_FCB){
            readfcb(fopt->fcb,nav);
        }
        else if(prcopt->arprod>=AR_PROD_OSB_GRM&&prcopt->arprod<=AR_PROD_OSB_CNT){
            readosb(fopt->bia,nav);
        }
        if(prcopt->arprod==AR_PROD_UPD){
            readupd(prcopt,fopt->updf[0],fopt->updf[1],fopt->updf[2],nav);
        }
    }
    ...
    for (i = 0; i < n; i++) {
        ...
        if (((ext = strrchr(infile[i], '.')) &&
            (!strcmp(ext, ".stat") || !strcmp(ext, ".STAT") ||
                ...
                !strcmp(ext, ".trp") || !strcmp(ext, ".TRP"))) || strstr(infile[i], "WRF")) {
            pppcorr_read(stas, &nav->pppcorr, infile[i]);
        }
    }
                      ...
                      obs_t *obs, nav_t *nav, sta_t *sta)
{
    ...
    for (i=0;i<n;i++) {
        ...
        if (readrnxt(prcopt,infile[i],rcv,ts,te,ti,prcopt->rnxopt[rcv<=1?0:1],obs,nav,
...
extern void freeobsnav(obs_t *obs, nav_t *nav)
{
    ...
    free(nav->seph); nav->seph=NULL; nav->ns=nav->nsmax=0;
    if(nav->fcbs){
        free(nav->fcbs->data);nav->fcbs->data=NULL;nav->fcbs->nmax=nav->fcbs->n=0;
    }
    if(nav->osbs){
        free(nav->osbs->sat_osb);nav->osbs->sat_osb=NULL;
    }
    if(nav->upds){
        free(nav->upds->nls.data); nav->upds->nls.n=nav->upds->nls.nmax=0;
        free(nav->upds);
    }
}
                  ...
                  const prcopt_t *opt)
{
    ...
    for (iobs=0;(m=nextobsf(obs,&iobs,rcv))>0;iobs+=m) {
        ...
        if (!pntpos(0,data,j,nav,opt,&sol,NULL,NULL,msg,NULL,NULL)) continue;
...
//        matcpy(pcvs->pcv[i].var[2],pcvs->pcv[i].var[1],80*50,1);
//    }
//    for (i=0;i<pcvr->n;i++) {
//        if (norm(pcvr->pcv[i].off[2],3)>0.0) continue;
//        matcpy(pcvr->pcv[i].off[2],pcvr->pcv[i].off[1], 3,1);
//        matcpy(pcvr->pcv[i].var[2],pcvr->pcv[i].var[1],80*50,1);
//    }
    return 1;
}
                   ...
                   const pcvs_t *pcvr, const sta_t *sta)
{
    ...
    for (i=0;i<(mode?2:1);i++) {
        ...
        if (!(pcv=searchpcv(0,popt->anttype[i],time,pcvs))) {
...
static int execses(gtime_t ts,gtime_t te,double ti,const prcopt_t *popt,
                   ...
                   char **infile, const int *index,const int n, const char *outfile)
{
    FILE *fp,*fptm;
    rtk_t rtk={0};
    prcopt_t popt_=*popt;
    ...
    /* open debug trace */
    if (flag&&sopt->trace<=5&&sopt->trace>=2) {
        if (*outfile) {
    ...
    /* read dcb parameters */
    if (*fopt->dcb&&(popt->cbiaopt==CBIAS_OPT_BRD_TGD||popt->cbiaopt==CBIAS_OPT_COD_DCB||popt->cbiaopt==CBIAS_OPT_MIX_DCB)) {
        reppath(fopt->dcb,path,ts,"","");
        readdcb(path,&navs,stas);
    }

    /* read mgex dcb */
    if(*fopt->mgexdcb&&(popt->cbiaopt==CBIAS_OPT_IGG_DCB||popt->cbiaopt==CBIAS_OPT_GBM_DCB||popt->cbiaopt==CBIAS_OPT_MIX_DCB)){
        readdcb_mgex(fopt->mgexdcb,popt,&navs);
    }

    /* set antenna parameters */
    if (popt_.mode!=PMODE_SINGLE&&*fopt->atx) {
    ...
    if(sopt->ambres&&popt_.modear==ARMODE_PPPAR_ILS){
    ...
    if(popt->modear>=ARMODE_OFF){
        fprintf(stderr,"Total epoch: %d(100.0%s), Fix epoch: %d(%3.1f%s), Else: %d\n",
                rtk.epoch,"%",rtk.fix_epoch,(rtk.fix_epoch+1e-9)/rtk.epoch*100.0,"%",rtk.epoch-rtk.fix_epoch);
        fflush(stderr);
    }
                     ...
                     const char *rov)
{
    gtime_t t0={0};
    int i,stat=0;
    char *ifile[MAXINFILE]={NULL},ofile[MAXSTRPATH]={'\0'},*rov_,*p,*q,s[128]="";
                     ...
                     const char *rov, const char *base)
{
    gtime_t t0={0};
    int i,stat=0;
    char *ifile[MAXINFILE]={NULL},ofile[1024]={'\0'},*base_,*p,*q,s[128];
                   ...
                   const char *rov, const char *base)
{
    ...
    double tunit,tss;
    int i,j=0,k,nf=0,stat=0,week,flag=1,index[MAXINFILE]={0};
    char *ifile[MAXINFILE]={NULL},ofile[1024]={'\0'},*ext=NULL;
