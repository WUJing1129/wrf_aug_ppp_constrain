...
extern double iontecvar(int ep,gtime_t t,const double *pos,const double *azel)
{
    double re=6378.0,hion=375.0;
    double blhp[3];
    double mf=ionppp(pos,azel,re,hion,blhp);
    ...
    /* local time (s) */
    double tt=43200.0*lam+time2gpst(t,NULL);
    tt-=floor(tt/86400.0)*86400.0; /* 0<=tt<86400 */
    tt/=3600.0;

    double var0=SQR(0.3),var;
    if(tt<8||tt>20||blhp[0]>PI/3.0){
        var=(1.0+0.2*ep)*var0*mf;
    }
    else{
        var0=(var0+var0*cos(blhp[0])*cos((tt-14)/12*PI));
        var=(1.0+0.2*ep)*var0*mf;
    }
    return var;
}